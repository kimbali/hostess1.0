...
const logic = {
    ...
    retrieveHostess(idH) {
        return Promise.resolve()
            .then(() => Hostess.findById(idH).populate({path: 'requests', select: { password: 0 }}).populate({path: 'accepted', select: { password: 0 }, populate: {path: 'events', populate: {path: 'business'}}}).populate({path: 'toConfirm', populate: {path: 'business', select: { password: 0 }}}).populate({path: 'toAssist', populate: {path: 'business', select: { password: 0 }}}).lean())
            .then(hostess => {
    ...
    retrieveEvent(idE) {
        return Promise.resolve()
            .then(() => Events.findById(idE).populate('business').populate('candidates').populate('approved').populate('confirmed').lean())
            .then(event => {
                if (!event) throw new LogicError('can not find the event')
                delete event.business.password
                delete event.__v
                return event
            })
    },
    ...
    editBusinessProfile(id, password, name, web, boss, phone, philosophy, businessCard) {
        return Promise.resolve()
            ...
            .then(business => {
                ...
                return this.saveImage(businessCard)
                .then(businessCard => {
    ...
    searchWorkers(idB, gender, languages, jobType) {
        return Promise.resolve()
            .then(() => {
                let criteria = {}
                ...
                return Hostess.find(criteria).lean()
                    .then(allHostesses => {
                        ...
                        if (allHostesses) {
                            ...
                            allHostesses.forEach(hostess => {
                                // hostess.id = hostess._id.toString()
                                // delete hostess._id
                                if(!(hostess.requests.indexOf(idB) !== -1)) hostesses.push(hostess)
    ...
    acceptRequest(idH, idB) {
        return Promise.resolve()
        .then(() => {
            return Hostess.updateOne({ _id: idH }, {$pull:{ requests: idB }, $push: { accepted: idB }})
        })
        .then(() => Hostess.findById(idH))
        .then(hostess => {

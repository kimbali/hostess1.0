...
router.post('/hostess-auth', jsonBodyParser, (req, res) => {
    ...
    logic.authenticateHostess(email, password)
        .then(id => {
            ...
            res.json({ message: 'hostess authenticated', token, id })
        })
        .catch(err => {
            const { message } = err
...
router.post('/business-auth', jsonBodyParser, (req, res) => {
    ...
    logic.authenticateBusiness(email, password)
        .then(id => {
            ...
            res.json({ message: 'business authenticated', token, id })
        })
        .catch(err => {
            const { message } = err
...
router.get('/event-details/:id/:idE', [validateJwt, jsonBodyParser], (req, res) => {
    const { params: { idE }} = req

    logic.retrieveEvent(idE)
    .then(event => res.status(200).json({ status: 'OK', event }))
    .catch(err => {
        const { message } = err
...
router.patch('/hostess-edit-password/:id', [validateJwt, jsonBodyParser], (req, res) => {
...
router.patch('/business-edit-password/:id', [validateJwt, jsonBodyParser], (req, res) => {
    const { params: { id }, body: { password, newPassword } } = req
...
router.patch('/hostess-update/:id', [validateJwt, jsonBodyParser], (req, res) => {
...
router.patch('/business-update/:id', [validateJwt, jsonBodyParser], (req, res) => {
 ...
 */
 ...
 router.get('/:id/search', [validateJwt, jsonBodyParser], (req, res) => {
    ...
    logic.searchWorkers(id, gender, languages, jobType)
        .then(hostesses => {
